...
from langchain_huggingface import HuggingFaceEmbeddings #model_name="sentence-transformers/all-MiniLM-L6-v2"
...
# Storing the vector embeddings in Pinecone vector space
vectorstore = PineconeVectorStore(index_name="auradocs-index",embedding=embeddings)
...
async def index_documents_async(documents: List[Document], batch_size: int = 50):
    ...
    print(f"VectorStore Indexing: Preparing to add {len(documents)} documents to vector store")
    ...
    # Process batches at the same time
    ...
    if successful == len(batches):
        print(f"VectorStore Indexing: All batches processed successfully! ({successful}/{len(batches)})")
    else:
...
async def main():
    ...
    print("TavilyCrawl: Starting to crawl the documentation site")
    ...
    for tavily_crawl_result_item in res["results"]:
        print(f"TavilyCrawl: Successfully crawled {tavily_crawl_result_item['url']} from documentation site")
        all_docs.append(
            Document(
    ...
    print(f"Text Splitter: Processing {len(all_docs)} documents with 4000 chunk size and 200 overlap")
